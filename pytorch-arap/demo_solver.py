...
def deform_cuboid():
    targ = os.path.join("sample_meshes", "cuboid.obj")
    meshes = load_objs_as_meshes([targ], load_textures=False)

    # meshes = ico_sphere(3)

    meshes = ARAP_from_meshes(meshes, device)  # convert to ARAP obejct
    ...
    ## plot mesh
    plot_meshes(ax, meshes.verts_list(), faces, handle_verts=handle_verts, static_verts=static_verts, color="gray",
                change_lims=True)
    ...
    def anim(i):
        ...
        ## deform, replot
        verts = meshes.solve(static_verts=static_verts, handle_verts=handle_verts, handle_verts_pos=handle_pos_shifted,
                     n_its=0)  ## run ARAP

        verts = [verts]

        trisurfs[:] = plot_meshes(ax, verts, faces, handle_verts=handle_verts, static_verts=static_verts)
...
def deform_sphere():
    ...
    trisurfs = plot_meshes(ax, meshes.verts_list(), faces, handle_verts=handle_verts, static_verts=static_verts, prop=False, change_lims=True,
                           color="gray")
    ...
    def anim(i):
        ...
        ## deform, replot
        verts = meshes.solve(static_verts=static_verts, handle_verts=handle_verts, handle_verts_pos=handle_pos_shifted, n_its = 1) ## run ARAP

        verts = [verts]

        trisurfs[:] = plot_meshes(ax, verts, faces, handle_verts=handle_verts, static_verts=static_verts, prop=False,
                                  color="gray")
