...
class ConstrainedLinearSolver { // solves linear system with some rows fixed
                                // (not unknown)
public:
  Eigen::MatrixXd xB;
  Eigen::VectorXi I, B;
  ...
  ConstrainedLinearSolver(const Eigen::SparseMatrix<double> &A,
    ...
    std::sort(range.begin(), range.end(),
              [&](const int i, const int j) { return constr[i] < constr[j]; });
    ...
    for (int i = 0; i < A.cols(); ++i)
      rangeFull.push_back(i);
    I.resize(A.cols() - constr.size());
    std::set_difference(rangeFull.begin(), rangeFull.end(), B.data(),
                        B.data() + B.size(), I.data());
...
// arguments: V holds original vertex positions, F faces, C cotan Laplace matrix
// and edgeSets_sr will hold resulting datastructure
void spokesAndRimsEdges_sr(Eigen::MatrixXd &V, Eigen::MatrixXi &F,
                           Eigen::MatrixXd C,
  ...
  edgeSets_sr.resize(nv); // one entry per vertex because per-vertex rotations
  ...
  for (int i = 0; i < nf; i++) { // go over triangles
    for (int e = 0; e < 3; e++) {
      ...
      int vj = F(i, j2);
      int vk = F(
          i, e); // opposite vertex (gives angle, identifies edge weight C(i,e))

      // push this edge with all 3 possible rotations and their corresponding
      // weight into datastructure
...
// arguments: matrix V0 holding original vertex positions, V1 current vertex
// positions edgeSets_sr holds information about mesh structure, has to be
// initialized via spokesAndRimsEdges_sr rot is a vector of 3x3 matrices, which
// is where the rotation will be stored
void findRotations_sr(const Eigen::MatrixXd &V0, const Eigen::MatrixXd &V1,
  ...
  flip(2, 2) = -1.;
  for (int i = 0; i < n; ++i) {
    Eigen::JacobiSVD<Eigen::Matrix3d> svd(rot[i], Eigen::ComputeFullU |
                                                      Eigen::ComputeFullV);
...
// eSets information about mesh structure, has to be initialized via
// spokesAndRimsEdges_sr L is cotan Laplace matrix, R holds rotations found via
// findRotations_sr lambda is smoothness parameter, rhs will hold the result of
// this function
void assembleRHS_sr(const Eigen::MatrixXd &V0, const Eigen::MatrixXd &LV0,
                    const std::vector<std::vector<Edge_sr>> &eSets,
                    const Eigen::SparseMatrix<double> &L,
                    const std::vector<Eigen::Matrix3d> &R, const double lambda,
                    Eigen::MatrixXd &rhs) {
  ...
  if (lambda) { // quadratic/smooth term
    ...
    for (int i = 0; i < b2.rows(); ++i) {
      b2.row(i) =
          R[i] * LV0.row(i).transpose(); // smooth arap has rotated lap vector
    }
...
// helper function: defines constraints for the survey experiments so they can
// be run simply via mesh name
void experiment(string mesh) {
  ...
  if (mesh.compare("square_21_spikes.off") == 0) { // spiked plane lifting
    ...
    Matrix3d def_r;
    def_r << 0.999641, 0.021031, 0.016579, -0.022776, 0.342021, 0.939416,
        0.014087, -0.939457, 0.342378;
    Vector3d def_t;
    def_t << -0.019322, -0.134331, 0.821073;
    std::vector<int> fixed = {
        200,  266,  276,  277,  288,  289,  290,  291,  292,  300,  304,  305,
        306,  308,  314,  315,  331,  332,  348,  350,  353,  354,  356,  358,
        473,  493,  496,  576,  577,  583,  584,  587,  588,  594,  595,  596,
        601,  602,  603,  604,  607,  609,  611,  612,  613,  614,  615,  616,
        617,  618,  619,  750,  758,  766,  767,  771,  774,  775,  779,  780,
        788,  791,  798,  801,  802,  803,  844,  1136, 1139, 1531, 1585, 1586,
        1587, 1630, 1631, 1648, 1649, 1650, 1682, 1684, 1691, 1692, 1693, 1694,
        404,  405,  406,  407,  408,  409,  410,  411,  412,  413,  414,  415,
        424,  425,  426,  427,  428,  429,  430,  431,  440,  441,  442,  443,
        444,  445,  446,  447,  448,  449,  450,  451,  452,  453,  454,  455,
        456,  457,  458,  459,  460,  461,  462,  463,  464,  465,  466,  467,
        468,  469,  470,  471,  472,  473,  474,  475,  476,  477,  478,  479,
        480,  481,  482,  483,  484,  485,  486,  487,  488,  489,  490,  491,
        492,  493,  494,  495,  496,  497,  498,  499,  500,  501,  502,  503,
        949,  950,  951,  952,  953,  979,  980,  981,  982,  983,  984,  1010,
        1011, 1012, 1013, 1014, 1015, 1042, 1043, 1044, 1045, 1046, 1268, 1269,
        1270, 1271, 1272, 1273, 1274, 1275, 1290, 1291, 1292, 1293, 1294, 1295,
        1296, 1297, 1298, 1299, 1300, 1301, 1302, 1303, 1312, 1313, 1314, 1315,
        1316, 1317, 1318, 1319, 1320, 1321, 1322, 1323, 1324, 1325, 1326, 1327,
        1328, 1329, 1330, 1331, 1332, 1333, 1334, 1335, 1336, 1337, 1338, 1339,
        1340, 1341, 1342, 1343, 1344, 1345, 1346, 1347, 1348, 1349, 1350, 1351,
        1352, 1353, 1354, 1355, 1356, 1357, 1358, 1359, 1360, 1361, 1362, 1363,
        1364, 1365, 1366, 1367, 1368, 1369, 1370, 1371, 1372, 1373, 1374, 1375,
        1376, 1377, 1378, 1379, 2196, 2197, 2198, 2199, 2200, 2201, 2202, 2203,
        2220, 2221, 2222, 2223, 2224, 2225, 2226, 2227, 2228, 2229, 2230, 2231,
        2232, 2233, 2234, 2235, 2244, 2245, 2246, 2247, 2248, 2249, 2250, 2251,
        ...
        2312, 2313, 2314, 2315, 3128, 3129, 3130, 3131, 3132, 3133, 3134, 3135,
        3144, 3145, 3146, 3147, 3148, 3149, 3150, 3151, 3166, 3167, 3168, 3169,
    ...
    std::vector<int> handle = {
        ...
        216,  217,  218,  219,  220,  221,  222,  279,  280,  281,  282,  283,
        284,  285,  286,  287,  288,  289,  290,  291,  292,  293,  294,  295,
        296,  297,  354,  355,  356,  357,  358,  359,  360,  361,  362,  363,
        364,  365,  366,  367,  368,  369,  370,  371,  372,  849,  850,  851,
        852,  853,  854,  855,  856,  857,  858,  859,  860,  861,  862,  863,
        864,  865,  866,  867,  1048, 1049, 1050, 1051, 1052, 1053, 1054, 1055,
        1056, 1057, 1058, 1059, 1060, 1061, 1062, 1063, 1064, 1065, 1066, 1067,
        1068, 1069, 1070, 1071, 1072, 1073, 1074, 1075, 1076, 1077, 1078, 1079,
        1080, 1081, 1082, 1083, 1084, 1085, 1086, 1087, 1088, 1089, 1090, 1091,
        1092, 1093, 1094, 1095, 1096, 1097, 1098, 1099, 1100, 1101, 1102, 1103,
        1104, 1105, 1106, 1107, 1108, 1109, 1110, 1111, 1112, 1113, 1114, 1115,
        1116, 1117, 1118, 1119, 1120, 1121, 1122, 2893, 2894, 2895, 2896, 2897,
        ...
        2970, 2971, 2972, 4384, 4385, 4386, 4387, 4388, 4389, 4390, 4391, 4392,
        ...
        4453, 5294, 5295, 5296, 5297, 5298, 5299, 5300, 5301, 5302, 5303, 5304,
    ...
    Matrix3d def_r;
    def_r << -0.5, 0.0, -0.866025, 0.0, 1.0, 0.0, 0.866025, 0.0, -0.5;
    Vector3d def_t;
    ...
    double t = 3.14;
    Matrix3d def_r;
    def_r << cos(t), 0.0, -sin(t), 0.0, 1, 0, sin(t), 0, cos(t);
    ...
    std::cout << "ISSUE: UNKNOWN EXPERIMENT. Constraints are unclear, please "
                 "use one of the survey experiments or specify your used "
                 "constraints in the code yourself."
              << std::endl;
  }
...
void bilap_init(std::vector<std::vector<Edge_sr>> edgeSets_sr) {
  A = L * M_inv * L;
  ConstrainedLinearSolver lapsolver(
      A, B, constrPositions); // build constrained solver for system matrix
  std::vector<Eigen::Matrix3d> rot(
      V.rows(), Eigen::Matrix3d::Identity()); // naive lap is identity
...
void new_experiment(string source_mesh_file_path,
                    string target_mesh_file_path) {
  Eigen::MatrixXd V_target;
  Eigen::MatrixXi F_target;

  igl::read_triangle_mesh(source_mesh_file_path, V, F);

  igl::read_triangle_mesh(target_mesh_file_path, V_target, F_target);

  std::cout << "source mesh loaded " << V.norm() << endl;
  std::cout << "target mesh loaded " << V_target.norm() << endl;

  for (int i = 0; i < int(V.rows() / 2); ++i) {
    B.push_back(i);
    Eigen::RowVector3d p = V_target.row(i);
    constrPositions.push_back(V_target.row(i));
  }
}
...
int main(int argc, const char *argv[]) {
  ...
  igl::massmatrix(V, F, igl::MASSMATRIX_TYPE_VORONOI,
                  M); // write into mass matrix
  area = M.diagonal().eval();
  // idea: normalize s.t. each area entry is approx 1 (divide by sum, all sum
  // to 1, times V which is number of areas)
  M /= area.sum();
  M *= V.rows();
  area /= area.sum(); // normalize to area 1 so smoothness lambda not
                      // dependent on size of mesh
  area *= V.rows();
  igl::invert_diag(M, M_inv); // invert it
  // load constraints from experiment, this is just needed to display them
  // later
  ...
  menu.callback_draw_viewer_menu = [&]() {
    ...
    if (ImGui::Combo("Initialization Scheme", &init_type,
                     "Handle\0Poisson\0Bi-Laplacian\0")) {
      ...
      if (init == BILAPLACIAN) {
        bilap_init(edgeSets_sr); // solve bi-laplacian (L^2) equation -> naive
                                 // laplacian editing
      }
    ...
    if (ImGui::Button("arap deformation", ImVec2(-1, 0))) {
      ...
      ConstrainedLinearSolver solver(
          A, B,
          constrPositions); // build constrained solver for system matrix
      if (triangle) {
        rot.resize(F.rows());
      }
      MatrixXd V_old = V;
      ...
      while (!converged && i < max_iters) {
        ...
        i++;
        if ((V1 - V_old).norm() / V.norm() <
            convergence_thresh) { // small relative change --> convergence
          converged = true;
        }
        V_old = V1; // update
      }
    ...
    if (ImGui::Button("save .obj and stats", ImVec2(-1, 0))) {
      // save mesh
      std::fstream s{"../res/" + save_name + ".obj",
                     s.binary | s.trunc | s.in | s.out};
      for (int i = 0; i < V.rows(); i++) {
        s << "v " << V1(i, 0) << " " << V1(i, 1) << " " << V1(i, 2)
          << std::endl;
      }
      for (int i = 0; i < F.rows(); i++) {
        s << "f " << F(i, 0) + 1 << "/" << F(i, 0) + 1 << " " << F(i, 1) + 1
      ...
      outFile << metric_iters << " iterations until convergence ("
              << convergence_thresh << ")" << endl;
      outFile << "took " << metric_time - metric_fac_time << " s, excluding "
              << metric_fac_time << " s for factorization" << endl;
  ...
  viewer.data().add_points(
      bc, point_color); // add points at constraints to see what was done
